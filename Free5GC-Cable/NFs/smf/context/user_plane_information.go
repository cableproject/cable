...
func (upi *UserPlaneInformation) GenerateNewPath(selection *UPFSelectionParams) bool {
	var source *UPNode
	var destinations []*UPNode
	...
	path, pathExist := getPathBetween(source, destinations[1], visited, selection)
...
func (upi *UserPlaneInformation) GetNewUserPlanePathByDNN(selection *UPFSelectionParams) (path UPPath) {
	pathExist := false
	logger.CtxLog.Traceln("In GetNewUserPlanePathByDNN")
	logger.CtxLog.Traceln("selection: ", selection.String())
	if pathExist {
		return
	} else {
		pathExist = upi.GenerateNewPath(selection)
		if pathExist {
			return upi.DefaultUserPlanePath[selection.String()]
		}
	}
