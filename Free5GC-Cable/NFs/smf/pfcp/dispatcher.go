...
func Dispatch(msg *pfcpUdp.Message) {
	...
	case pfcp.PFCP_UPF_REPORT_REQUEST:
		handler.HandlePfcpUpfReportRequest(msg)
	case pfcp.PFCP_UPF_REPORT_RESPONSE:
		handler.HandlePfcpUpfReportResponse(msg)
	case pfcp.PDU_SESSION_DATA_REPORT_RESPONSE:
		handler.HandlePduSessionDataReportResponse(msg)
	case pfcp.PDU_SESSION_RELEASE_FROM_SMF_REQUEST:
		handler.HandlePDUSessionReleaseFromSMFRequest(msg)
	default:
