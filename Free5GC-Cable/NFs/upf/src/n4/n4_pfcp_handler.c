...
Status UpfN4HandleUPFReportRequest(PfcpXact *xact, PFCPUPFReportRequest *request) {
	Status status;
    PfcpHeader header;
    Bufblk *bufBlk = NULL;

    UTLT_Info("[PFCP] UPF Report Request");
 
    /* Send */
    memset(&header, 0, sizeof(PfcpHeader));
    header.type = PFCP_UPF_REPORT_RESPONSE;
    header.seid = 0;

    status = UpfN4BuildUPFReportResponse(&bufBlk, header.type);
    UTLT_Assert(status == STATUS_OK, return STATUS_ERROR,
             "N4 build error");
    ...
    UTLT_Info("[PFCP] UPF Report Response");
    return STATUS_OK;
}

Status UpfN4HandlePDUSessionDataReportRequest(PfcpXact *xact, PFCPPDUSessionDataReportRequest *request, uint16_t i) {
    Status status;
    PfcpHeader header;
    Bufblk *bufBlk = NULL;

    UTLT_Info("[PFCP] PDU Session Data Report Request");

    /* Send */
    memset(&header, 0, sizeof(PfcpHeader));
    header.type = PFCP_PDU_SESSION_DATA_REPORT_RESPONSE;
    header.seid = 0;

    status = UpfN4BuildPDUSessionDataReportResponse(&bufBlk, header.type, i);
    UTLT_Assert(status == STATUS_OK, return STATUS_ERROR,
             "N4 build error");
    ...
    UTLT_Info("[PFCP] PDU Session Data Report Response");
    return STATUS_OK;
}
