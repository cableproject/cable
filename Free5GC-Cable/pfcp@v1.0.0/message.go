...
func (message *Message) IsRequest() (IsRequest bool) {
	...
	case PFCP_UPF_REPORT_REQUEST:
		IsRequest = true
	case PDU_SESSION_DATA_REPORT_REQUEST:
		IsRequest = true
	case PDU_SESSION_RELEASE_FROM_SMF_REQUEST:
		IsRequest = true
	default:
		IsRequest = false
	}
...
func (message *Message) IsResponse() (IsResponse bool) {
	...
	case PFCP_SESSION_REPORT_RESPONSE:
		IsResponse = true
	case PFCP_UPF_REPORT_RESPONSE:
		IsResponse = true
	case PDU_SESSION_DATA_REPORT_RESPONSE:
		IsResponse = true
	case PDU_SESSION_RELEASE_FROM_SMF_RESPONSE:
		IsResponse = true
	default:
		IsResponse = false
	}
...
type PFCPUPFReportRequest struct {
	//request结构
	NodeID *pfcpType.NodeID `tlv:"60"`
}

type PFCPUPFReportResponse struct {
	//response结构
	NodeID           *pfcpType.NodeID   `tlv:"60"`
	UPFcpu           *pfcpType.UPFDataF `tlv:"203"`
	UPFmem           *pfcpType.UPFDataF `tlv:"204"`
	UPFsto           *pfcpType.UPFDataF `tlv:"205"`
	UPFUplinkData    *pfcpType.UPFData  `tlv:"198"`
	UPFDownlinkData  *pfcpType.UPFData  `tlv:"199"`
	PFCPUplinkData   *pfcpType.UPFData  `tlv:"200"`
	PFCPDownlinkData *pfcpType.UPFData  `tlv:"201"`
	Timestamp        *pfcpType.UPFDataT `tlv:"202"`
}

type PDUSessionDataReportRequest struct {
	//request结构
	NodeID *pfcpType.NodeID `tlv:"60"`
}

type PDUSessionDataReportResponse struct {
	//response结构
	NodeID    *pfcpType.NodeID    `tlv:"60"`
	PDUTEID   *pfcpType.PDUTEID   `tlv:"206"`
	PDUUEIP   *pfcpType.PDUUEIP   `tlv:"207"`
	PDUPacket *pfcpType.PDUPacket `tlv:"208"`
	Timestamp *pfcpType.UPFDataT  `tlv:"202"`
}

type PDUSessionReleaseFromSMFRequest struct {
	PDUSessionID *pfcpType.PDUSessionID
}

type PDUSessionReleaseFromSMFResponse struct {
	Cause *pfcpType.Cause `tlv:"19"`
}
