...
func (m *Message) Unmarshal(data []byte) error {
	...
	case PFCP_UPF_REPORT_REQUEST:
		Body := PFCPUPFReportRequest{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
	case PFCP_UPF_REPORT_RESPONSE:
		Body := PFCPUPFReportResponse{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
	case PDU_SESSION_DATA_REPORT_REQUEST:
		Body := PDUSessionDataReportRequest{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
	case PDU_SESSION_DATA_REPORT_RESPONSE:
		Body := PDUSessionDataReportResponse{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
	case PDU_SESSION_RELEASE_FROM_SMF_REQUEST:
		Body := PDUSessionReleaseFromSMFRequest{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
	case PDU_SESSION_RELEASE_FROM_SMF_RESPONSE:
		Body := PDUSessionReleaseFromSMFResponse{}
		if err := tlv.Unmarshal(data[m.Header.Len():], &Body); err != nil {
			return err
		}
		m.Body = Body
